...
class pathPlanning():
    ...
    def subCallback(self, data):
        ...
        # conter variable is used to store the initial values.
        if self.times_ran == 0:
    ...
    def pathDefinition(self):
        # this routine makes the car travel the pre-defined path
        if abs(self.distance)<0.2:
            ...
            # this conditional block reorients the car and resets initial values for calculations
            if self.path == 1 and self.times_ran == 2:
                self.service_call(self.current_angular_z+30)
                self.initial_x = self.current_x
                self.initial_y = self.current_y
                
            elif self.path == 2 and self.times_ran == 2:
                self.service_call(self.current_angular_z-30)
                self.initial_x = self.current_x
                self.initial_y = self.current_y
            ...
            if self.times_ran == 3:
                ...
                self.initial_y = self.current_y
            self.carControl()
            self.distance = self.distanceCalculation(self.line) + 0.2
            
        else:
            # reset all velocity commands
            self.velocityValue()
    ...
    def carControl(self):
        ...
        if self.line == 'straight':
            ...
            travel_range_ul = 0.10
            travel_range_ll = -0.10
        # this if block checks if the car is moving whithin the error limits.
